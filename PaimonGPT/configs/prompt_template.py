...
multiqueryretriever_prompt_template = """问句：{query}。
分析上面的问句，从多种角度生成最多3个相似问句。
使用 json 格式回答，确保你的回答必须是正确的 json 格式，并且能被 python 语言的 `json.loads` 库解析。格式如下：
{{"question1":"question1", "question2":"question2", "question3":"question3"}}
"""

tableQA_prompt_template = """你是一个专业的表格数据分析师。根据给定的表格数据来回答用户的问题。
用户的问题是：{query}
表格数据是：
{context}
"""
